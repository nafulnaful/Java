...
<script type="text/javascript">
	...
	function updatePage(){
		...
		if(req.readyState==4 && req.status==200){
			...
			//alert(res);//XMLDocument객체로 응답 온다.
			//원하는 데이터를 추출해야 한다.
			//res==>XMLDocument==> $()함수로 wrapping하면 dom탐색을 이용해서 원하는 데이터를 추출할 수 있다.
			let midx=$(res).find('idx').text();
			//alert(midx+"/"+typeof midx);
			let mname=$(res).find('name').text();
			let maddr=$(res).find('addr').text();
